...
#define THREESECONDDELAY            UINT32_C(10) /**< one second is represented by this macro */
...
//                (long int) getaccelData.xAxisData, (long int) getaccelData.yAxisData, (long int) getaccelData.zAxisData);
    	//this will temporily not use, but it will be used later.
    	long int xacc=getaccelData.xAxisData;
    	long int yacc=getaccelData.yAxisData;
    	if (abs(xacc)<30)xacc=0;
    	if (abs(yacc)<30)yacc=0;
    	xspeed=xacc*0.01+xspeed;
    	yspeed=yacc*0.01+yspeed;

    	speed = sqrt(xspeed*xspeed+yspeed*yspeed);
    	speed_2=sqrt(xspeed*xspeed+yspeed*yspeed);

    	distance_2= speed_2*0.01+distance_2;
    	distance=distance_2;
		xAccelData=getaccelData.xAxisData;
		yAccelData=getaccelData.yAxisData;
		zAccelData=getaccelData.zAxisData;


        xaccelold=xaccel;
        yaccelold=yaccel;
        zaccelold=zaccel;
        xaccel=getaccelData.xAxisData;
        yaccel=getaccelData.yAxisData;
		zaccel=getaccelData.zAxisData;

		temp=sqrt((xaccel-xaccelold)^2+(yaccel-yaccelold)^2+(zaccel-zaccelold)^2);
		if (temp >30)
		{
			crash+=1;
		}
