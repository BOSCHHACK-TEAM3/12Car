...
#define TIMERDELAY       UINT32_C(100)          /** Three second delay is represented by this macro */
...
void printMagneticData(xTimerHandle pxTimer)
{
    ...
    if ( RETCODE_OK == sensorApiRetValue)
    {
        ...
        xm = (long int) getMagData.xAxisData;
        ym = (long int) getMagData.yAxisData;
        zm = (long int) getMagData.zAxisData;
        xm=xm-45;
        ym=ym-10;
               double angle=0;
        if (xm!=0)
        {
        	...
        	if((xm<0)||(ym>0))angle=180-angle;
        	if((xm<0)||(ym<0))angle=180+angle;
        	if((xm>0)||(ym<0))angle=360-angle;
        }
        else
        {
        	if(ym>0) angle=90;
